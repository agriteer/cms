...
class MailService extends Mail
{
    ...
    {
        try {
            $from = ['email' => 'support@agriteer.com', 'name' => 'Agriteer Support'];
    ...
    {
        try {
            $to = ['email' => 'support@agriteer.com', 'name' => 'Agriteer Support'];
            $from = ['email' => $email->email, 'name' => $vendor->name];
            ...
            return $this->send('noreply@agriteer.com', $yser->email, $subject, $message);
        } catch (\Exception $e) {
            return ['message' => 'Error sending email'];
        }
    }
